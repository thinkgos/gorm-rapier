...
func Test_Columns_SubQuery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "in sub query",
			db: newDb().Model(xDict.X_Model()).
				Where(xDict.Id.IntoColumns().In(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id)).Where(xDict.Score.Gt(100)))).
				Find(&dummy),
		...
		},
		{
			...
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns(xDict.Id, xDict.Name).In(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id, xDict.Name)).Where(xDict.Score.Gt(100)))).
				Find(&dummy),
			wantVars: []any{float64(100)},
		...
		},
		{
			...
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns().In(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id)).Where(xDict.Score.Gt(100)))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE ",
		},
		...
		},
		{
			...
			db: newDb().Model(xDict.X_Model()).
				Where(xDict.Id.IntoColumns().NotIn(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id)).Where(xDict.Score.Gt(100)))).
				Find(&dummy),
			wantVars: []any{float64(100)},
		...
		},
		{
			...
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns().NotIn(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id)).Where(xDict.Score.Gt(100)))).
				Find(&dummy),
			wantVars: nil,
			...
			db: newDb().Model(xDict.X_Model()).
				Where(
					xDict.Id.IntoColumns().Eq(
						newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Max())),
					),
				).
				Find(&dummy),
			wantVars: nil,
		...
		},
		{
			name: "neq",
			db: newDb().Model(xDict.X_Model()).
				Where(xDict.Id.IntoColumns().Neq(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Max())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE `dict`.`id` <> (SELECT MAX(`dict`.`id`) FROM `dict`)",
		},
		{
			name: "neq(no fields)",
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns().Neq(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Max())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE ",
		},
		{
			name: "gt",
			db: newDb().Model(xDict.X_Model()).
				Where(xDict.Id.IntoColumns().Gt(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
		...
		},
		{
			name: "gt - (no field)",
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns().Gt(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE ",
		},
		{
			name: "gte",
			db: newDb().Model(xDict.X_Model()).
				Where(xDict.Id.IntoColumns().Gte(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE `dict`.`id` >= (SELECT MIN(`dict`.`id`) FROM `dict`)",
		},
		{
			name: "gte - (no field)",
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns().Gte(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE ",
		},
		{
			name: "lt",
			db: newDb().Model(xDict.X_Model()).
				Where(xDict.Id.IntoColumns().Lt(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE `dict`.`id` < (SELECT MIN(`dict`.`id`) FROM `dict`)",
		},
		{
			name: "lt - (no field)",
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns().Lt(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE ",
		},
		{
			name: "lte",
			db: newDb().Model(xDict.X_Model()).
				Where(xDict.Id.IntoColumns().Lte(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE `dict`.`id` <= (SELECT MIN(`dict`.`id`) FROM `dict`)",
		},
		{
			name: "lte - (no field)",
			db: newDb().Model(xDict.X_Model()).
				Where(NewColumns().Lte(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
			want:     "SELECT * FROM `dict` WHERE ",
		},
		{
			name: "exist",
			db: newDb().Model(xDict.X_Model()).
				Where(Exist(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
		...
		},
		{
			name: "not exist",
			db: newDb().Model(xDict.X_Model()).
				Where(NotExist(newDb().Scopes(xDict.Xc_Model(), Select(xDict.Id.Min())))).
				Find(&dummy),
			wantVars: nil,
